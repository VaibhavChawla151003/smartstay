...
export default function PieRoomsCost() {
  const rooms = useSelector((state)=>state.listings)
  ...
  useEffect(() => {
    ...
    rooms.forEach((room) => {
      ...
      if (room.price <= 50) return between35And50++;
      moreThan50++;
    });
    setCostGroups([
      { name: 'Less Than $15', qty: lessThan15 },
      { name: 'Between $15 & $35', qty: between15And35 },
      { name: 'Between $35 & $50', qty: between35And50 },
      { name: 'More than $50', qty: moreThan50 },
    ]);
  }, [rooms]);
  return (
    <Box
    ...
    >
      ...
      <Stack gap={2}>
        <Typography variant="h6">Listings Cost</Typography>
